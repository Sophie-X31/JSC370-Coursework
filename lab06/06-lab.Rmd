...
#counts <- xml2::xml_find_first(website, "/html/body/main/div[9]/div[2]/div[2]/div[1]/h3/span")

# Turning it into text
#counts <- as.character(xml2::xml_text(counts))
...
institution <- str_extract_all(
  publications_txt,
  #"Univeristy\\s+of\\s+[[:alpha:]]+|[[:alpha:]]+\\s+Institute\\s+of\\s+[[:alpha:]]+"
  "University\\s+of\\s+[A-Za-z]+|[A-Za-z]+\\s+Institute\\s+of\\s+[A-Za-z]+"
  ) 
...
schools_and_deps <- str_extract_all(
  ...
  "School\\s+of\\s+[A-Za-z]+|Department\\s+of\\s+[A-Za-z]+"
  )
...
```
_There are 15 publications without an abstract._
...
#titles <- str_remove_all(abstracts, "\\s+")
...
```{r}
write.csv(database, "pubmed", row.names = TRUE)
```
